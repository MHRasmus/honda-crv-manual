...
function Apf()
{
	var  ppp;
	var  strClick;
	for(ppp=0;ppp<parent.window.HEAD.document.all.length;ppp++) {
		pobj = parent.window.HEAD.document.all(ppp);
		if(pobj.onclick != null) {
			var str = pobj.src.split("print");
			if(str.length > 1) {
	...
	var  para = "ChangeCursor('\"+save_cursor+\"')";
	setTimeout(para,100);
...
function StandardPrint()
{
	...
	if(gIdx_ > 0) {
		...
		parent.resizeTo(pageWidth,orgHei);
